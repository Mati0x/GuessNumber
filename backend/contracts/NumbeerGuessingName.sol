...
contract NumberGuessingGame {
    ...
    modifier setPrizeAmount(){
        require(msg.value == PRIZE_AMOUNT, "Must pay 1 ether to continue..");
        _;
    }
    ...
    modifier betPoolIs1Ether(){
          require(betPool >= 0.1 ether, "Bet pool has not reached the minimum threshold");
        _;
    }
    ...
    function setMisteryNumber(uint256 _misteryNumber) external payable onlyOwner setPrizeAmount  {
        require(misteryNumber == 0, "Target number has already been set");
        require(_misteryNumber >= 1 && _misteryNumber <= 1000, "Number must be between 1 and 1000");
    ...
    function guessNumber(uint256 number) external payable numberNotZero  {
        ...
        require(number >= 1 && number <= 1000, "Number must be between 1 and 1000");
    ...
    function resetGame(uint256 _misteryNumber) external payable onlyOwner setPrizeAmount  {
        misteryNumber = _misteryNumber;
        prizePool = PRIZE_AMOUNT;
        betPool = 0;
        delete allGuesses;
